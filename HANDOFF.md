...
- **Empty-slot placeholders** — affordances like "text…" inside an empty `TextBox.content`, or per-slot prompts inside an empty named slot, so users can see and click into slots that have no content yet. The selection model already supports slot-kind targets (landed in click-into-named-slots); placeholders are the visual + dispatch piece that lights up empty slots — DOM-instrumented via the loader (it knows when a slot has no fill), placeholder element makes the slot region clickable.
