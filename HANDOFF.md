...
- **`slidewright/`** — the Slidewright package.
  ...
  - **`canvas/`** — host-agnostic canvas UI.
    ...
    - `selection-layer.tsx` — React component that renders all selection visuals (per-shape outlines, group bbox, handles). Iterates the loader's shape registry. Branches per item: shapes use `adapter.calculateBounds` from gesture-adjusted params; layouts (HStack / VStack) DOM-measure bounds via the loader's wrapper. Portals via `useSelectionPortal` (see `portal-target.ts`).
    - `portal-target.ts` — `useSelectionPortal` hook that finds the right portal target for selection visuals + gesture overlays: closest Freeform's positioned div if there is one, slide stage (`.presentation-canvas`) otherwise. Shared by `selection-layer.tsx` + `gesture-overlay-layer.tsx`.
...
**Slide-level layouts (no Freeform ancestor) now work.** `slidewright/canvas/portal-target.ts:useSelectionPortal` does a conditional lookup: prefer the closest Freeform's positioned div (preserves the existing coord system for shapes + layouts inside Freeforms), fall back to the slide stage (`.presentation-canvas`) for layouts at slide-level. Selection visuals + gesture overlays portal into whichever ancestor is appropriate. `stack-adapter.ts:findReferenceFrame` mirrors the same fallback for measurement, so DOM measurements stay in the same frame as rendering. The `body: VStack { ... }` pattern (existing slide 3 in the reference deck) and the rebuilt "Stacks demo" slide both work now.
