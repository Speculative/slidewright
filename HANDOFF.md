...
- **`slidewright/`** — the Slidewright package.
  ...
  - **`canvas/`** — host-agnostic canvas UI.
    ...
    - `portal-target.ts` — `findPortalAncestor(el)` non-hook helper + `useSelectionPortal` hook for finding the right portal target: closest Freeform's positioned div if there is one, slide stage (`.presentation-canvas`) otherwise. Shared by `selection-layer.tsx`, `gesture-overlay-layer.tsx`, `stack-adapter.ts`.
