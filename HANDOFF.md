...
- **Empty-slot placeholders Phase B (intentional-empty sigil)** — Phase A always-on placeholders landed; Phase B introduces an explicit "this slot is meant to be empty" sigil in source so the user can suppress placeholders for slots they've acknowledged. Components can also offer custom treatments of the sigil (e.g., a CardRow could hide its border entirely instead of showing one with no content). Needs (a) source grammar for the sigil — likely `slotName:` with a designated literal like `none`, or a no-value form; (b) loader distinguishing "no fill at all" from "explicit-empty"; (c) suppression rule (parent-selection-only or fully hidden); (d) inspector affordance to toggle a slot between always-show / explicit-empty. Until Phase B lands, Phase A's always-on placeholders create some layout pressure on slides with many optional slots — acceptable for editing, would surprise in presentation if there were a separate presentation mode (currently editor IS presentation).
