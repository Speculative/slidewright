...
### Gesture model and shape adapters **DECIDED (v0.3)**

The architecture is React-native: gesture state is React state; per-pointermove `setState` updates dx/dy; the loader wraps each shape with a `ShapeProjection` that consumes a gesture context and re-renders the shape with adjusted params; selection visuals (outlines, group bbox, handles, marquee preview, create preview) are React components portaled into the active Freeform. No imperative DOM mutation during drag.
...
The earlier (v0.2) architecture was imperative: adapters' `onMove` mutated `style.left` / `style.top` per frame. Selection outlines and group bbox were `document.createElement`'d in effects and tracked via separate "during-drag update" code sites. This produced a recurring bug class — every new visual that depended on shape position needed a separate "remember to update X during drag" hookup, and we kept forgetting them (per-shape outline tracking, arrow endpoint handles tracking, group bbox tracking each landed as separate fixes).

React-native eliminates the bug class: every visual is a pure function of `(source params + active delta)`. Adding a new gesture-following visual is "render it as a React component reading the gesture context"; it can't lag during drag because its position isn't separately maintained.

Performance was the concern with per-pointermove `setState`. Validated empirically: at 60fps target, slide-scale shape counts (~50–500) sit comfortably under 16.67ms per frame. The cliff is around N=2000 (`tests/perf-stress.spec.ts`), well past anything Slidewright will encounter.

**App-owned dispatch with co-located shape adapters.** The canvas (`slidewright/canvas/App.tsx`) is the gesture dispatcher. It owns the pointer-event lifecycle, the gesture mutex (single `gesture` state slot), the source round-trip pipeline, and tool-mode policy. Per-shape behavior lives on a `canvas: ShapeAdapter` export co-located with each shape's component file (`decks/v0-reference/components/Box.tsx` etc.).

**Why App-owned dispatch instead of per-shape pointer handlers:**

Mutex of "only one gesture at a time" is a single state slot, not a context provider every shape consults. Tool-mode policy ("if `activeTool=box`, pointerdown on a Box should *not* drag — it should start a new Box-creation gesture") lives in one place. Cross-shape gestures (multi-select, snap, alignment) are tractable because App sees all shapes. Source-update cancellation is automatic via React effect cleanup.

**Co-location vs separation of adapters:**

Adapters live with their components (`Box.tsx` exports both `slidewright` metadata and `canvas` adapter), not in a separate `slidewright/canvas/shapes/` directory. There's exactly one canvas-like surface in Slidewright; co-location wins on cohesion. The runtime layer (`LoadedComponent`) carries the adapter via an opaque `canvas?: unknown` field so the runtime stays canvas-agnostic — the canvas casts to the real shape.
...
Source spans shift every time the canonical formatter rewrites whitespace. Drag and resize commits stash the new shapes' spans in a `pendingSelectionRef[]` before calling `host.setSource`; the subscribe handler picks them up on the round-trip and re-applies. The mechanism: capture each shape's *child index* in its parent Freeform's children list before emit, look up the same index in the freshly parsed post-emit AST, read its new span.

**Test strategy: end-to-end, not unit.**

Gesture tests run in real Chromium via Playwright (`tests/gestures.spec.ts`, `tests/editor-sync.spec.ts`), driving the standalone canvas with a real mouse and asserting on the source visible in the editor pane after the round-trip. Pure-function tests of the AST-mutation logic alone would miss the bug classes that actually happen here: pointer-event wiring, React effect ordering, contentEditable focus / blur, transform-stuck-on-rerender. The e2e suite survives refactors as black-box behavior validation. **DECIDED.**
...
```
parse(source) → mutate AST in place → emit() → reparse for new selection spans → setSource
```

Centralized in `slidewright/canvas/ast-edits.ts:commitSourceEdit`. Takes a `mutate` callback that operates on the freshly parsed AST and returns either `null` (abort), `{}` (commit without selection preservation), or `{ preserveSelections: PreserveSelection[] }` (commit and have the helper compute new selection spans post-emit). Returns `{ newSource, newSelections: SourceRange[] }` for the caller to apply. Multi-shape gestures (group drag, group delete) populate `preserveSelections` with one entry per affected shape.
...
### Known design concerns and outside-Freeform generalization **OPEN**

Concerns within the current Freeform-bound architecture, captured so future work doesn't re-litigate. None are blockers at current scale; each has a known mitigation path when it bites.

**Per-frame full-tree re-render.** `setGestureLive` changes the GestureProvider's value, fires every `ShapeProjection`'s `useShapeDelta` consumer regardless of whether that shape is in the gesture's targets. Non-affected shapes return null, params pass through unchanged, but the function component still runs. Slide-scale (~50–500 shapes) is invisible; we measured the cliff around N=2000. Mitigations when needed: `useContextSelector`-style selector hook, per-shape `React.memo` with custom prop equality, or splitting GestureProvider into per-span contexts (overkill).

~~**SelectionLayer's portal target via DOM lookup.**~~ Resolved in v0.4: `slidewright/canvas/portal-target.ts:useSelectionPortal` does a conditional lookup — prefer the closest Freeform's positioned div, fall back to the slide stage (`.presentation-canvas`) when there's no Freeform ancestor. Selection visuals + gesture overlays now work for layouts at any depth, including layouts at slide-level (`body: VStack { ... }`). The Freeform-anchored path is unchanged for shapes; existing coord semantics preserved. Cross-context coord comparison (e.g., distance between a stage-level layout and a Freeform-internal shape) isn't directly supported but isn't needed by current features — both paths render at correct slide-design positions.

**`gestureMeta` + `gestureLive` split is non-obvious.** Two pieces of state for what's conceptually one gesture, gated on the perf concern that `setGesture(prev => …)`-per-frame would re-fire the gesture-lifecycle effect (cleanup + re-attach document listeners ~60×/sec). The split is justified but adds a tiny ceremony at every gesture-start path (set both, clear both). A future consolidation that skips the perf optimization would only cost per-frame remove-and-add of two listeners — measurable but not user-visible at slide scale.

~~**DOM-measured bounds break the "pure function of state" invariant.**~~ Resolved. LayoutAdapter's bounds (and gap-drag's grip positions) are DOM-measured rather than computed from params — layouts are flow-laid by their parent, params don't determine bounds. The original v0.4 implementation used per-consumer `useLayoutEffect` plus dep lists that had to enumerate every state input affecting layout, which produced three regressions from the same root: inline-in-render measurement reading pre-commit DOM (timing); missing `gestureDeltas` dep skipping re-measurement during gap-drag (deps); no-deps `useLayoutEffect` with early-return paths creating new identities and looping forever (bailout). Fixed structurally by `slidewright/canvas/layout-measurement.ts` — single primitive (`measureLayoutSnapshot` for one-shot reads, `useLayoutMeasurement` for reactive subscription) with `ResizeObserver` per layout. Observer fires on layout commits regardless of which React state triggered them; consumers pull from the snapshot. Decouples bounds-tracking from React's state graph entirely.

**Latent: ResizeObserver doesn't follow element-identity through unmount/remount.** The observer subscribes to a specific element reference. When React replaces a tracked layout's DOM with a new instance (same conceptual element, different identity), the observer keeps pointing at the detached old element and goes silent. Cached state stays stale until the consumer's `useEffect` re-runs and re-subscribes. We've shipped one regression from this (selection outline disappeared after slide nav-away-and-back, because the standalone canvas only mounts the active slide; old layout DOM unmounted, observer didn't follow). Fixed by `key={activeIdx}` on `<SelectionLayer>` + `<GestureOverlayLayer>`, which forces a remount on slide nav and re-establishes observers on whatever's currently mounted. **Discipline rule:** any new state that causes element-identity changes for a tracked layout must propagate to the consumer's effect deps OR trigger a remount via `key`. Currently covered: `selected`, `shapes` (source change), `portalTarget`, `activeIdx` (slide nav). If a fourth regression from this expanded root appears, refactor to a `MutationObserver` on the slide stage that detects DOM mutations and re-establishes ResizeObservers on the now-mounted layouts — generic; handles arbitrary future remount scenarios at the cost of an additional subscription model layered on top of ResizeObserver.

Outside-Freeform generalization — the **substantive** concerns. The current `ShapeAdapter` contract assumes shapes have absolute design-space coords (Box's `x, y` or Arrow's `x1, y1, x2, y2`). Generalizing to Card / Stack-children / Diagram-internal shapes runs into:

- **Layout-driven shapes don't have a `params → bounds` pure function.** A Card inside a CardRow has `width` in its params, but `x, y` are flow-laid by the parent. `calculateBounds(params)` doesn't fit. Two design candidates: (a) framework reads `getBoundingClientRect` post-render and stashes in `ShapeData`, adapter receives bounds rather than computing them — reintroduces a DOM dependency we just dropped; (b) adapter declares "I'm flow-laid" via a capability flag, framework computes base bounds from DOM, adapter applies delta-induced bounds changes (resize) on top. Either fix is fine for one shape kind; uniform treatment matters when we have a mix.
- **Pin gestures don't fit the current commit model.** Per "Direct manipulation of structured component output" below, pins are the primary mechanism for nudging children inside structured components — drag writes a *parent*-stored override, not a child-param mutation. The current `commit(ast, span, delta, slideIdx)` assumes the dragged shape's slot fills change. Probable fix: layout primitives get their own adapter that intercepts gestures originating from descendants — "Diagram captures drags within it; Card just signals 'I was dragged' and Diagram writes the pin." That's a meaningful contract addition (parent-ownership of descendant gestures).
- **Reorder gestures aren't translates.** Drag a child within an HStack → preview is translate-flavored, but the commit is a list-position change. Could fit as a new `ReorderDelta` variant. The drop-zone indicator (UI showing "release here to insert at position N") is a separate visual that lives on the *parent*, not the dragged child — another pressure point on the layout-primitive-adapter idea.
- **Mutex with nested gesture-able contexts.** Card inside Diagram inside Freeform: clicking the Card → which adapter's gesture wins? `target.closest(SELECTABLE_SELECTOR)` already picks the innermost, which is the right rule. Mostly a non-issue once we have layout-primitive adapters; flagging in case it isn't.
- **`ShapeProjection` wraps every component, not just adapter-having ones.** Today harmless (adapter-less wrappers pass params through); for future-larger trees, optimize the loader to skip the wrapper for components without a `canvas` adapter.

**Recommendation: defer outside-Freeform until an actual outside-Freeform shape needs canvas behavior.** The two big design questions (DOM-based vs param-based bounds, parent-owned vs child-owned commits) are choices the contract will need to make, and making them speculatively will land in the wrong place. When a Card with resize handles, a sortable HStack child, or a Diagram with internal nodes lands as a real requirement, that's the design pass — and it'll inform whether we go DOM-bounds, layout-aware adapters, parent-ownership, or some mix.
...
**v0.2.j — Multi-select. Status: implemented.**
- Selection state is `selected: SourceRange[]` (within one layout context; cross-context multi-select forbidden).
- Shift-click toggles selection of an individual shape.
...
- Group body-drag translates all selected shapes by the same delta.
- `commitSourceEdit` accepts multiple `preserveSelections`.
- Group delete iterates and removes.
- Selection rendering: dashed outline per selected shape + minimum-bounding-box rendered when 2+ are selected.
- Handles only render when exactly one shape is selected. Group resize is deferred.

**v0.3 — Slot-aware editing + inspector + external-edit reconciliation. Status: gesture refactor implemented; remainder of scope still open.**

The React-native gesture refactor was prioritized after v0.2.j to eliminate a recurring "visual didn't update during drag" bug class. See Editor / Gesture model and shape adapters for the full architecture. Highlights:
...
- Selection visuals (outlines, group bbox, marquee preview, create preview, handles) are React components portaled into the active Freeform.
- Adapter contract is fully declarative: `boundsFromParams`, `applyGesture`, `Handles`, `commit`. No imperative DOM mutation.
- Per-frame `setState` validated to sustain 60fps to N=1000 shapes (perf sweep at `tests/perf-stress.spec.ts`, opt-in via `npm run test:perf`).
- Box and TextBox share `makeRectAdapter({ width, height })`. New HTML-rectangle shape primitives are two lines.

Still open from the original v0.3 scope:
...
**Resolved during v0.2 / v0.3 implementation:**
- ~~Per-shape canvas behavior architecture~~ → `ShapeAdapter` contract co-located with shape components; App.tsx is a thin dispatcher. App-owned dispatch (vs per-shape pointer handlers) won on mutex / tool-mode policy / cross-shape gesture composability.
- ~~Selection preservation across emit cycles~~ → child-index-in-parent-list lookup (spans shift, indices don't); `pendingSelectionRef` carries the post-emit spans (array — supports group commits) back through the source round-trip.
...
- ~~Imperative-during-drag vs React-driven gestures~~ → React-native. Gesture state is React state; `setState` per pointermove; loader's `ShapeProjection` consumes a gesture context and re-renders shapes with `adapter.applyGesture`-adjusted params; selection visuals are React components portaled into the active Freeform. Eliminates a recurring "visual didn't update during drag" bug class. Performance validated to N=1000 shapes at 60fps.
