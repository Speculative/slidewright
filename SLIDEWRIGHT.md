...
**SelectionLayer's portal target via DOM lookup.** `useFreeformDiv` walks `closest('[data-sw-component="Freeform"]')` to find the active slide's Freeform's positioned div. Works because of the multi-select-within-one-Freeform rule; if we ever want selection visuals anchored to a non-Freeform ancestor (when generalizing beyond Freeform), the lookup breaks silently. Future fix likely: framework computes the portal target per shape (its closest positioned ancestor) and caches in the shape registry.
