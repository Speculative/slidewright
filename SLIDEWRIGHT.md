...
**DOM-measured bounds break the "pure function of state" invariant.** ShapeAdapter has a clean property — bounds come from `calculateBounds(params)`, a pure function of React state — so visuals depending on bounds re-derive automatically when state changes. LayoutAdapter doesn't get this: layouts are flow-laid, params don't determine their bounds, and the framework DOM-measures them via `getBoundingClientRect` (in `selection-layer.tsx:useMeasuredLayoutBounds` and `stack-adapter.ts:useMeasuredGripPositions`). DOM measurements read the *output* of rendering, not the inputs, which means consumers must (a) defer measurement to `useLayoutEffect` (after the commit phase, before paint) and (b) include every state input that could affect layout in their effect's deps. We've shipped two regressions from this same root: (1) inline-in-render measurement reading pre-commit DOM during the v0.4 tight cut, fixed by moving to `useLayoutEffect`; (2) the bounds effect missing `gestureDeltas` from deps, so it didn't re-measure during gap-drag, fixed by adding the dep. **Discipline rule for new gestures**: when adding a visual that depends on layout-measured state, audit every state input that could affect that layout's size — selection, source, gesture-adjusted params, scale — and ensure it's in the measurement effect's deps. If a third regression from this root lands, refactor to a `ResizeObserver`-driven measurement: framework subscribes to layout shifts on each registered layout once; consumers pull the latest measurement without manual dep plumbing. Decouples bounds-tracking from React's state graph entirely, at the cost of an additional subscription model.
